...
class AppSheets {
  ...
  static Future<T> showAppHeightNineSheet<T>(
    ...
    if (removeUntilHome) {
      return Navigator.pushNamed(context, '/dd');
    }
    return Navigator.push<T>(context, route);
  }
  ///App ten sheet
  static Future<T> showAppHeightTenSheet<T>(
      {@required BuildContext context,
      @required WidgetBuilder builder,
      Color color,
      double radius = 30.0,
    ...
    var route = _AppHeightTenModalRoute<T>(
        builder: builder,
        color: color,
        radius: radius,
        barrierLabel:
...
class _AppHeightTenSheetLayout extends SingleChildLayoutDelegate {
  _AppHeightTenSheetLayout(this.progress);
  ...
  @override
  bool shouldRelayout(_AppHeightTenSheetLayout oldDelegate) {
    return progress != oldDelegate.progress;
  }
}//end
class _AppHeightTenModalRoute<T> extends PopupRoute<T> {
  _AppHeightTenModalRoute(
      {this.builder,
      this.barrierLabel,
      this.color,
      this.radius,
      ...
      child: GestureDetector(
        ...
        child: Theme(
          ...
          child: AnimatedBuilder(
            animation: appSheetAnimation,
            builder: (context, child) => CustomSingleChildLayout(
                  delegate: _AppHeightTenSheetLayout(appSheetAnimation.value),
                  child: BottomSheet(
                    animationController: _animationController,
                    onClosing: () => Navigator.pop(context),
                    builder: (context) => Container(
